...
2. Area of a Parallelogram from its Defining Vectors
   ...
   For example, if $$\vec{u} = \begin{pmatrix} a \\ b \end{pmatrix}$$ and $$\vec{v} = \begin{pmatrix} c \\ d \end{pmatrix}$$, then $$M = \begin{pmatrix} a & c \\ b & d \end{pmatrix}$$, and the area is $$ \vert ad - bc \vert $$.
