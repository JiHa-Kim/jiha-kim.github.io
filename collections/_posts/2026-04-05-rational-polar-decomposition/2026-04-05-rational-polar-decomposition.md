...
In the Gram-space iteration ($B = A^\top A$), we use a **Resolvent-Basis Expansion** that guarantees exact stability while cutting the matrix multiplication FLOPs in half. Because the algebra generated by $B$ and the bounded resolvent $H_0 = (I + cB)^{-1} = \gamma(\gamma I + B)^{-1}$ closes linearly under the identity $B H_0 = (I - H_0)/c = \gamma(I - H_0)$, we can analytically expand the DWH update $B \leftarrow B (\alpha I + \frac{\beta}{\gamma} H_0)^2$ into a linear combination that avoids matrix cross-multiplications entirely:
$$
B \leftarrow c_I I + c_B B + c_{H} H_0 + c_{H2} H_0^2
$$
By using this algebraic flattening, we compute the DWH update using only one symmetric matrix squaring, $H_0^2$. This is structurally executed via a Symmetric Rank-K (`SYRK`) update, which takes exactly half the memory bandwidth and FLOPs of the general `SYMM` required by the unexpanded representation. The scale-invariant coefficients $c_I, c_B, c_H, c_{H2}$ are pre-computed in FP64, eliminating all dynamic-range runtime evaluation.
...
Because minimax polynomials aggressively push the spectrum exactly to $1.0$, floating point iterations can suffer from catastrophic cancellation when computing additions like $1.0 - 0.9999$. We sidestep this completely by operating in a **Pure Defect Pipeline**. By analytically mapping the Gram system to its defect form $E = I - B$, all $O(1)$ scalar components perfectly annihilate. The entire normalized PE sequence structurally executes as a root-finding iteration purely on the $E$ tensor, guaranteeing robust error resolution in FP16/BF16 without relative truncation bounds.
...
| :------- | :-------------------------- | :------------------------------------------------------------ |
| **DWH**  | $c_I, c_B, c_H, c_{H2}$     | $0.030883301527615, 0.968872554082809, 3.906861822017413, -3.937745123545028$ |
|          | $\alpha_0, \tilde{\beta}_0$ | $0.984313239818915, 251.007791810856$                         |
| **PE 1** | $\delta_1, \hat{c}_1$       | $1.595552602479211, 3.901806628246143$                         |
