...
Recent work on GPU-friendly polar/sign methods in ML makes the same point: high accuracy is often unnecessary, while GEMM-only iterations and fast early progress matter [2].
...
This is in the spirit of matrix equilibration / diagonal scaling methods [4] used to reduce extreme row/column scaling while preserving symmetry.
...
This uses Gershgorin's theorem: all eigenvalues lie within Gershgorin discs (intervals for symmetric matrices), so the quantity above provides a cheap, conservative lower bound; shifting by <span class="math-inline" markdown="0">\(\delta I\)</span> increases all eigenvalues. [5]
...
This "minimax polynomial per step" mechanism is exactly the conceptual link to Polar Express [2].
...
- [1] Nicholas J. Higham. "Newton's method for the matrix square root." *Numerische Mathematik*, 48(1):97–117, 1986.
- [2] Noah Amsel, David Persson, Christopher Musco, Robert Gower. "The Polar Express: Optimal Matrix Sign Methods and Their Application to the Muon Algorithm." arXiv:2505.16932 (2025).
- [3] Peihua Li et al. "Towards Faster Training of Global Covariance Pooling Networks by Iterative Matrix Square Root Normalization." arXiv:1712.01034 (2018).
- [4] Daniel Ruiz. "A Scaling Algorithm to Equilibrate Both Rows and Columns Norms in Matrices." RAL-TR-2001-034 (2001).
- [5] Gershgorin circle theorem (standard result on eigenvalue inclusion intervals).
