...
> In 1D, optimal transport was monotone: equal quantiles never cross. In higher dimensions, the analogue is that a tiny material segment should not get folded backward by the map. Indeed, convexity gives
> $$ \langle \nabla\psi(x)-\nabla\psi(y),\, x-y \rangle \ge 0, $$
> so the image separation has nonnegative projection along the original separation. The segment may stretch and rotate, but its shadow on the original direction never points backward. When $D=1$, gradients of convex functions are exactly increasing functions, so Brenier reduces to monotone rearrangement.

The cleanest way to see this is to restrict everything to one short segment. Along that line, convexity becomes an ordinary 1D increase in projected motion, so the shadow stays forward:
