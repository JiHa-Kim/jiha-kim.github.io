...
> [!remark] The Right Higher-Dimensional Analogue of Monotonicity
...
> $$
> \sum_{i=1}^m \langle x_i, T(x_i)\rangle
> \ge
> \sum_{i=1}^m \langle x_i, T(x_{i+1})\rangle,
> \qquad x_{m+1}=x_1.
> $$
> For quadratic cost, this is equivalent to
> $$
> \sum_{i=1}^m \|x_i-T(x_i)\|_2^2
> \le
> \sum_{i=1}^m \|x_i-T(x_{i+1})\|_2^2.
> $$
> So no finite cyclic reassignment of destinations can lower the total transport cost.

> [!remark] Pairwise Monotonicity Is Only the 2-Cycle Case
> Taking $m=2$ gives
> $$
> \langle T(x_1)-T(x_2),\,x_1-x_2\rangle \ge 0,
> $$
> which is the familiar monotonicity inequality. That is necessary, but Brenier's structure is stronger: it rules out *every* finite cyclic swap, not just pairwise reversals.

> [!theorem] Rockafellar's Characterization {% cite rockafellarCharacterizationSubdifferentialsConvex1966 %}
...
> $$
> \Gamma \subset \partial \phi.
> $$
> In particular, if $\Gamma$ is the graph of a single-valued map $T$, then $T(x)\in \partial \phi(x)$, and wherever $\phi$ is differentiable we have $T(x)=\nabla\phi(x)$.
...
{% include brenier_monotonicity_widget.html %}
